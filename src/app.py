...
def show_updates_list(updates):
    ...
    for n, u in enumerate(updates, start=1):
        print(f"{n:>4}. {u[0]} ({str(round(u[1] / 1_000_000, 1))} MB)")
    print(f"Total size: {get_total_size(updates) // 1_000_000} MB")

def main():
    start_dir = Path(sys.argv[1])
    if not start_dir.is_dir():
        print(f'Error: Not a valid folder: {start_dir}')
        exit(1)
    # Find Logos.exe first to find install dir "Logos".
    logos_exe = utils.get_logos_exe(start_dir)
    if logos_exe is None:
        print(f"Error: Logos.exe not found anywhere in {start_dir}")
        exit(1)
    ...
    if user_id is None:
        print(f"Error: Logos user has not logged in.")
        exit(1)
    # Get DB files.
